 ...
 */
	...
	Element.prototype.animate = function (definition, options) {
		...
		if (!player.finished) createFinishedPromise(player);

		// polyfill problematic/unimplemented properties if present
		if (!from.clipPath) return player;
		...
		if ('duration' in options) this.style.transitionDuration = options.duration + 'ms';
		if ('easing' in options) this.style.transitionTimingFunction = options.easing;
		...
		player.finished.then(() => {
			...
			// remove 
			if (options.fill != 'forwards') properties.forEach(property => this.style[property] = '');
		});
